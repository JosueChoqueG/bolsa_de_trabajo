...
function messagesXhr(xhr, textStatus = null) {
    switch (xhr.status) {
        ...
        case 500:
            ...
            if(xhr.responseJSON.message == "23000")
                msgInfo('El elemento está en uso y no se puede eliminar.');
            else
                msgWarning('No se pudo realizar la operación. inténtelo otra vez');
            break;
        case 588:
                msgWarning('No se pudo establecer conexion con la SUNAT');
                break;
        default:
...
function loginWeb(url,data)
{
    $.ajax({
        ...
        beforeSend: function( xhr ) {
            ...
            $('.message_login').html('Verificando credenciales...');
        },
        success  : function(response)
        {
           if(response.status == 'success'){
                $('.btn-login').prop('disabled',true);
                $('.message_login').html('redireccionando...');
                msgInfo(response.message,'');
                if( $selected_card != null){
                    setTimeout(function(){ 
                        $selected_card.trigger('click');
                        $selected_card = null; 
                    }, 1200); 
                }
                else{
               ...
               msgError(response.message,'Error de identificacion');
           }     
        },
        error: function(xhr, textStatus, thrownError)
        {    
