...
class CandidateUpdateRequest extends FormRequest
{
    ...
    {
        return [    
            'document'        => 'bail|required|digits:8|unique:candidate,document,'.$this->route('candidate_id').',id',
            ...
            'email'           => 'bail|required|email|max:50|unique:candidate,email,'.$this->route('candidate_id').',id',
            ...
            'item_codigo.*'   => 'bail|required|numeric|unique:candidate_college,code,'.$this->route('candidate_id').',candidate_id',
            'item_carrera_id.*'   => 'bail|required|exists:college_career,id',
            'item_ingreso.*'  => 'bail|required',
        ];
    }
}
